...
This dataset contains the anthropometric measurements collected by a health professional during visits V0, V2, and V3. File `anthropo.tsv` stores the following information:
...
### Lifestyle
Information from the lifestyle survey is stored at `lifestyle.tsv`:
...
### Health
File `health.tsv` stores manually-acquired information about frequent medication intake and current diseases:
...
## DS6: Vital Signs
The vital signs measured by a health professional during the nutritionist visits are stored in `vitalsigns.tsv`: 
